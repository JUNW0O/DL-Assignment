...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        ###################################
        out_ch, in_ch, f_h, f_w = self.W.shape
        batch_size, in_ch, in_h, in_w = x.shape

        out_h = 1 + in_h - f_h
        out_w = 1 + in_w - f_w

        y = view_as_windows(x, (1, in_ch, f_h, f_w))
        y = y.reshape((batch_size, 1, out_h, out_w, 1, -1))

        filt = self.W.reshape((1, out_ch, 1, 1, -1, 1))
        bias = self.b.reshape((1, out_ch, 1, 1, -1, 1))

        # torch.squeeze can squeeze batch_size if batch_size = 1
        out = torch.reshape(torch.matmul(y, filt) + bias, (batch_size, out_ch, out_h, out_w))
        return out
        ###################################
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        ###################################
        batch_size, ch_size, in_h, in_w = x.shape
        out_h = int(1 + (in_h - self.pool_size) / self.stride)
        out_w = int(1 + (in_w - self.pool_size) / self.stride)

        pool_window = (1, 1, self.pool_size, self.pool_size)
        pool_step = (1, 1, self.stride, self.stride)
        y = view_as_windows(x, pool_window, pool_step)

        y = y.reshape(batch_size, ch_size, out_h, out_w, -1)

        out, _ = torch.max(y, dim=-1)
        return out
        ###################################
